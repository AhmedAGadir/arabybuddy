...
│   ├── welcome.tsx             # → export { WelcomeScreen } from '@/features/auth'
│   ├── signin.tsx
│   ├── signup.tsx
│   ├── forgot-password.tsx
│   ├── reset-password.tsx
...
│   ├── auth/
│   │   ├── components/         # UI components (AuthButton/, PasswordInput/, etc.)
│   │   ├── screens/            # Screen components
│   │   ├── hooks/              # useAuthContext, etc.
│   │   ├── providers/          # AuthProvider
│   │   ├── services/           # authService, oauthService.native/web
...
**React components should be "dumb" - they handle UI only, not business logic.**

Components should only:
- Manage UI state (loading, error display, form inputs)
- Call service methods
- Render UI based on props/state

Business logic belongs in:
- `services/` - API calls, data transformations, auth flows
...
### Fonts & Arabic Content

Default fonts are applied automatically via Gluestack UI components:
- `<Heading>` uses Luckiest Guy (English display font)
- `<Text>` uses Inter (English body font)

For Arabic text, add the appropriate font class:

```tsx
// Arabic headings
<Heading className="font-heading-ar">مرحبا</Heading>
<Heading className="font-heading-ar-bold">عنوان عريض</Heading>

// Arabic body text
<Text className="font-body-ar">نص عربي</Text>
...
| Purpose | Font | Class |
|---------|------|-------|
| English Headings | Luckiest Guy | (default) |
| Arabic Headings | DG Bebo | `font-heading-ar` |
| English Body | Inter | (default) |
