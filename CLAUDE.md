...
arabybuddy/
│
├── app/                                    # EXPO ROUTER - Thin routes only (NO business logic!)
│   ├── _layout.tsx                         # Root layout (providers, navigation config only)
...
│   ├── auth/
│   │   └── callback.tsx                    # → export { AuthCallbackScreen } from '@/features/auth'
│   └── (tabs)/
│       ├── _layout.tsx                     # Tab navigator config (navigation only)
...
2. **No Business Logic in `app/`**: The `app/` folder is **only for routing**. All business logic, components, hooks, and services must live in `features/` or `shared/`. This includes:
   - Screen components with UI and logic → `features/*/screens/`
   - API calls and data fetching → `features/*/services/`
   - Form validation → `features/*/validation/`
   - Custom hooks → `features/*/hooks/`
   
   **Exception**: Layout files (`_layout.tsx`) may contain minimal navigation configuration.

3. **Feature Encapsulation**: Each feature exports a clean public API via `index.ts`:
   ```typescript
...
5. **Shared Only When Cross-Cutting**: Only truly shared utilities go in `shared/`
...
1. Run linting: `npm run lint` and fix all errors and warnings
2. Run type check: `npm run typecheck` and fix all errors and warnings
3. Run tests: `npm run test` and fix all errors and warnings
